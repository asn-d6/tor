...
helper_create_origin_circuit(int purpose, int flags)
{
  ...
  circ->hs_ident = tor_malloc_zero(sizeof(hs_circ_identifier_t));
  circ->hs_ident->auth_key_type = HS_AUTH_KEY_TYPE_ED25519;
...
helper_create_service(void)
{
  ...
  tt_assert(service);
  service->version = HS_VERSION_THREE;
...
helper_create_service_ip(void)
{
  ...
  hs_service_intro_point_t *ip = service_intro_point_new();
  tt_assert(ip);
...
test_service_intro_point(void *arg)
{
   ...
   * created object (non legacy). */
  {
    ...
    query = service_intro_point_find(service, ip->auth_key_kp.pubkey.pubkey,
                                     ip->base.is_only_legacy);
    tt_mem_op(query, OP_EQ, ip, sizeof(hs_service_intro_point_t));
    query = service_intro_point_find(service, garbage, 0);
    tt_assert(query == NULL);
    /* Test lookup by circuit identifier. */
    ident.auth_key_type = HS_AUTH_KEY_TYPE_ED25519;
    ed25519_pubkey_copy(&ident.intro_key.ed25519_pk, &ip->auth_key_kp.pubkey);
    query = service_intro_point_find_by_ident(service, &ident);
    tt_mem_op(query, OP_EQ, ip, sizeof(hs_service_intro_point_t));
    ...
    query = service_intro_point_find(service, ip->auth_key_kp.pubkey.pubkey,
                                     ip->base.is_only_legacy);
    tt_assert(query == NULL);
    /* Garbage auth key in the ident. */
    setup_full_capture_of_logs(LOG_WARN);
    ident.auth_key_type = HS_AUTH_KEY_TYPE_ED25519;
    memset(&ident.intro_key.ed25519_pk, 'A', ED25519_PUBKEY_LEN);
    query = service_intro_point_find_by_ident(service, &ident);
    tt_assert(query == NULL);
    expect_log_msg_containing("Unknown authentication key on the "
                              "introduction circuit for service");
    teardown_capture_of_logs();
  }

  /* Test a legacy intropoint.  */
  {
    hs_service_intro_point_t *query;
    uint8_t garbage[DIGEST256_LEN] = {0};
    hs_circ_identifier_t ident = {0};

    /* Transform the IP into a legacy one. */
    ip->legacy_key = crypto_pk_new();
    int ret = crypto_pk_generate_key(ip->legacy_key);
    tt_int_op(ret, OP_EQ, 0);
    ip->base.is_only_legacy = 1;
    /* Add intropoint to descriptor map. */
    service_intro_point_add(service->desc_current->intro_points.map, ip);
    query = service_intro_point_find(service, ip->legacy_key,
                                     ip->base.is_only_legacy);
    tt_mem_op(query, OP_EQ, ip, sizeof(hs_service_intro_point_t));
    query = service_intro_point_find(service, garbage, 0);
    tt_assert(query == NULL);
    /* Can't find it with a non legacy lookup. */
    query = service_intro_point_find(service,
                                     ip->auth_key_kp.pubkey.pubkey, 0);
    tt_assert(query == NULL);
    /* Test lookup by circuit identifier. */
    ident.auth_key_type = HS_AUTH_KEY_TYPE_LEGACY;
    ident.intro_key.legacy = ip->legacy_key;
    query = service_intro_point_find_by_ident(service, &ident);
    tt_mem_op(query, OP_EQ, ip, sizeof(hs_service_intro_point_t));
    ...
    setup_full_capture_of_logs(LOG_WARN);
    ident.auth_key_type = HS_AUTH_KEY_TYPE_LEGACY;
    ident.intro_key.legacy = crypto_pk_new();
    query = service_intro_point_find_by_ident(service, &ident);
...
test_helper_functions(void *arg)
{
  ...
  /* Testing can_service_launch_intro_circuit() */
  {
    ...
    service->state.num_intro_circ_launched = MAX_INTRO_CIRCS_PER_PERIOD;
    ret = can_service_launch_intro_circuit(service, now);
    tt_int_op(ret, OP_EQ, 0);
  }
...
test_intro_circuit_opened(void *arg)
{
  ...
  /* Set an IP object now for this circuit. */
  {
    ...
    ed25519_pubkey_copy(&circ->hs_ident->intro_key.ed25519_pk,
...
test_service_event(void *arg)
{
   ...
   * here that should get cleaned up. */
  {
    ...
    tt_int_op(digest256map_size(service->desc_current->intro_points.map),
              OP_EQ, 1);
    tt_int_op(ip->circuit_established, OP_EQ, 0);
...
test_build_update_descriptors(void *arg)
{
  ...
  tt_int_op(service->desc_current->num_established_intro_circ, OP_EQ, 0);
  ...
  tt_int_op(service->desc_next->num_established_intro_circ, OP_EQ, 0);
  ...
  tt_int_op(digest256map_size(service->desc_next->intro_points.map),
            OP_EQ, 0);
  /* The revision counter is updated because we did try to update it. */
  tt_u64_op(service->desc_current->desc->plaintext_data.revision_counter,
            OP_EQ, 1);
  tt_u64_op(service->desc_next->desc->plaintext_data.revision_counter,
            OP_EQ, 1);
  ...
  expect_log_msg_containing("just picked 1 intro points. "
  ...
  /* Second update of this descriptor, revision counter should follow. The
   * next upload time is still unset because we actually want to upload this
   * descriptor so we keep it below "now". */
  tt_u64_op(service->desc_current->desc->plaintext_data.revision_counter,
            OP_EQ, 2);
...
test_upload_desctriptors(void *arg)
{
  ...
  service->desc_current->num_established_intro_circ = 1;
  service->desc_next->num_established_intro_circ = 1;
  ...
  setup_full_capture_of_logs(LOG_WARN);
  run_upload_descriptor_event(now);
  expect_log_msg_containing("No nodes found supporting v3 HSDir. Skipping.");
  teardown_capture_of_logs();
  tt_u64_op(service->desc_current->next_upload_time, OP_EQ,
            now + service->config.descriptor_post_period);
