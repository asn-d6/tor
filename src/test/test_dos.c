...
test_dos_conn_creation(void *arg)
{
  ...
  MOCK(cc_is_enabled, mock_enable_dos_protection);
  MOCK(conn_is_enabled, mock_enable_dos_protection);
  ...
  tt_int_op(DOS_CONN_DEFENSE_NONE, OP_EQ, dos_conn_permits_address(&addr));

  /* Register another conn and check that new conns are not allowed anymore */
  dos_new_client_conn(&addr);
  tt_int_op(DOS_CONN_DEFENSE_CLOSE, OP_EQ, dos_conn_permits_address(&addr));
...
test_dos_circuit_creation(void *arg)
{
  ...
  tt_int_op(DOS_CC_DEFENSE_NONE, OP_EQ,
            dos_cc_assess_circuit(TO_CIRCUIT(circ)));

  /* Register another CREATE cell that will push us over the limit. Check that
   * the cell gets refused. */
  dos_cc_new_create_cell(chan);
  tt_int_op(DOS_CC_DEFENSE_REFUSE_CELL, OP_EQ,
            dos_cc_assess_circuit(TO_CIRCUIT(circ)));
...
struct testcase_t dos_tests[] = {
  { "dos_conn_creation", test_dos_conn_creation, TT_FORK, NULL, NULL },
  { "dos_circuit_creation", test_dos_circuit_creation, TT_FORK, NULL, NULL },
  END_OF_TESTCASES
};
