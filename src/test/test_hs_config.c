...
test_invalid_service(void *arg)
{
  ...
  /* Try with a missing port configuration. */
  {
    ...
    expect_log_msg_containing("HiddenServiceVersion be between 2 and 3");
  ...
  /* Bad value of HiddenServiceAllowUnknownPorts. */
  {
    ...
    ret = helper_config_service(conf, 1);
    tt_int_op(ret, OP_EQ, -1);
    expect_log_msg_containing("HiddenServiceAllowUnknownPorts should be "
                              "0 or 1, not 2");
    teardown_capture_of_logs();
  }
  ...
  /* Bad value of HiddenServiceDirGroupReadable */
  {
    ...
    ret = helper_config_service(conf, 1);
    tt_int_op(ret, OP_EQ, -1);
    expect_log_msg_containing("HiddenServiceDirGroupReadable should be "
                              "0 or 1, not 2");
    teardown_capture_of_logs();
  }
  ...
  /* Bad value of HiddenServiceMaxStreamsCloseCircuit */
  {
    ...
    ret = helper_config_service(conf, 1);
    tt_int_op(ret, OP_EQ, -1);
    expect_log_msg_containing("HiddenServiceMaxStreamsCloseCircuit should "
                              "be 0 or 1, not 2");
    teardown_capture_of_logs();
  }
