...
/**
 * Return true iff a and b have the same strings in the same order.
 */
static int
smartlists_str_eq(const smartlist_t *a, const smartlist_t *b)
{
  if (!a) {
    return (!b);
  }
  ...
  if (smartlist_len(a) != smartlist_len(b))
    return 0;
  SMARTLIST_FOREACH_BEGIN(a, const char *, a_str) {
    const char *b_str = smartlist_get(b, a_str_sl_idx);
    if (strcmp(a_str, b_str)) {
      return 0;
    }
  } SMARTLIST_FOREACH_END(a_str);
  return 1;
}
                                  ...
                                  const or_options_t *new)
{
  ...
  return
    (old->UseEntryGuards != new->UseEntryGuards ||
     ...
     !smartlists_str_eq(old->FirewallPorts, new->FirewallPorts) ||
