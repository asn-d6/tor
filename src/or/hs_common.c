 ...
 *
 * checksum_out must be large enough to receive HS_SERVICE_ADDR_CHECKSUM_LEN
 * bytes. */
static void
...
{
  size_t offset = 0;
  char data[HS_SERVICE_ADDR_CHECKSUM_LEN];
  ...
  tor_assert(offset == HS_SERVICE_ADDR_CHECKSUM_LEN);
...
/* Using a binary representation of a service address, parse its content into
 ...
 * bytes long. */
static void
parse_hs_address(const char *address, ed25519_public_key_t *key_out,
...
hs_address_is_valid(const char *address)
{
  ...
  parse_hs_address(decoded, &key, checksum, &version);
...
/* Build hs_index which is used to find the responsible hsdirs.
...
/* Build hsdir_index which is used to find the responsible hsdirs.
 ...
 * and put their node_t object in the responsible_dirs list. If is_next_period
 * is true, the next hsdir_index of the node_t is used. If is_client is true,
 * the spread fetch consensus parameter is used else the spread store is used
 * which is only for upload. This function can't fail but it is possible that
 * the responsible_dirs list contains fewer nodes than expected.
 *
 * This function goes over the nodelist and sorts it by hsdir_index then does
 * a binary search to find the closest node. All of this makes it a bit CPU
 * intensive so use it wisely. */
void
                          ...
                          int is_client, smartlist_t *responsible_dirs)
{
   ...
   * XXX: Using smartlist_add_all() would be faster but would result in a lot
   * of nodes not supporting v3 and thus doing more back and forth during the
   * bsearch below. */
  SMARTLIST_FOREACH_BEGIN(nodelist_get_list(), node_t *, n) {
    if (n->rs->supports_v3_hsdir) {
      smartlist_add(sorted_nodes, n);
    }
  } SMARTLIST_FOREACH_END(n);
  if (smartlist_len(sorted_nodes) == 0) {
    log_warn(LD_REND, "No nodes found supporting v3 HSDir. Skipping.");
    goto done;
  }
  ...
  for (int replica = 1; replica <= hs_get_hsdir_n_replicas(); replica++) {
    ...
    while (n_added < n_to_add) {
      ...
      tor_assert(node->rs->supports_v3_hsdir);
      ...
      if (!smartlist_contains(responsible_dirs, node)) {
        smartlist_add(responsible_dirs, node);
        ++n_added;
      }
      if (++idx == smartlist_len(sorted_nodes)) {
...
/* Release and cleanup all memory of the HS subsystem. This is called by
 * tor_free_all(). Only for >= v3. */
void
hs_free_all(void)
{
