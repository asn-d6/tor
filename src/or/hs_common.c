 ...
 * checksum_out MUST at least be 2 bytes long. */
static void
                          ...
                          int is_client, smartlist_t *responsible_dirs)
{
   ...
   * hsdir_index already computed for them for this consensus. */
  {
    ...
    SMARTLIST_FOREACH_BEGIN(c->routerstatus_list, const routerstatus_t *, rs) {
       ...
       * we can't add const object in a smartlist. */
 ...
 *
 *   key = base32(hsdir identity) + base32(hs_identity)
...
hs_purge_hid_serv_from_last_hid_serv_requests(const char *req_key_str)
{
  ...
  for (iter = strmap_iter_init(last_hid_serv_requests);
    ...
    /* Do basic length check (both should be nul-terminated strings here) */
    if (strlen(key) < strlen(req_key_str)) {
      log_warn(LD_GENERAL, "Got request shorter than key");
      iter = strmap_iter_next(last_hid_serv_requests, iter);
      continue;
    }

    /* Check if the tracked request matches our request key */
    if (!strcmpend(key, req_key_str)) {
      iter = strmap_iter_next_rmv(last_hid_serv_requests, iter);
      tor_free(val);
    } else {
      iter = strmap_iter_next(last_hid_serv_requests, iter);
...
hs_pick_hsdir(smartlist_t *responsible_dirs, const char *req_key_str)
{
  ...
  SMARTLIST_FOREACH_BEGIN(responsible_dirs, routerstatus_t *, dir) {
    time_t last = hs_lookup_last_hid_serv_request(dir, req_key_str,
                                               0, 0);
