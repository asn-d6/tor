                       ...
                       time_t now)
{
  ...
  /* XXX chill the log severities. */
  log_warn(LD_GENERAL, "Seen client from '%s' with transport '%s'.",
...
geoip_get_transport_history(void)
{
  ...
  log_warn(LD_GENERAL, "Starting iteration for transport history. %d clients.",
  ...
  HT_FOREACH(ent, clientmap, &client_history) {
    ...
    if (!smartlist_string_isin(transports_used, transport_name))
      smartlist_add(transports_used, tor_strdup(transport_name));

    log_warn(LD_GENERAL, "Client from '%s' with transport '%s'. "
  ...
  SMARTLIST_FOREACH_BEGIN(transports_used, const char *, transport_name) {
    ...
    log_warn(LD_GENERAL, "We got %u clients with transport '%s'.",
  ...
  log_warn(LD_GENERAL, "Final bridge-ip-transports string: '%s'", the_string);
