                           ...
                           hostname_type_t addresstype)
{
  ...
  if (addresstype == ONION_V2_HOSTNAME) { /* it's a v2 hidden service */
    ...
    if (cached_desc) {
      ...
      descriptor_is_usable = hs_any_intro_points_usable(cached_desc);
      log_warn(LD_GENERAL, "Found usable desc for %s. No need to fetch.",
               onion_address);
    } else {
  ...
  if (refetch_desc || !descriptor_is_usable) {
    ...
    if (addresstype == ONION_V2_HOSTNAME) {
      ...
      hs_client_refetch_v3_renddesc(&edge_conn->hs_ident->identity_pk);
    }
    return 0;
  }
