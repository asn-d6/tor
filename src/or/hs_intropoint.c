...
/* We just received an INTRODUCE1 cell on <b>client_circ</b> with the payload
 * in <b>request</b> of size <b>request_len</b>. Return 0 if everything went
 * well, or -1 if an error occured. This function is in charge of sending back
 * an INTRODUCE_ACK cell and will close client_circ on error. */
STATIC int
...
/* Identify if the encoded cell we just received is a legacy one or not. */
STATIC int
