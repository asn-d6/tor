...
static hs_service_intro_point_t *
service_intro_point_find(const hs_service_t *service, const void *auth_key,
                         int is_legacy)
{
  uint8_t key[DIGEST256_LEN] = {0};
                                  ...
                                  const hs_ident_circuit_t *ident)
{
  ...
  case HS_AUTH_KEY_TYPE_LEGACY:
    ip = service_intro_point_find(service, ident->auth_rsa_pk, 1);
    break;
  case HS_AUTH_KEY_TYPE_ED25519:
    ip = service_intro_point_find(service, &ident->auth_ed25519_pk, 0);
    break;
  default:
    tor_assert(0);
  }
                         ...
                         hs_service_descriptor_t *desc, int for_next_period)
{
  ...
  SMARTLIST_FOREACH_BEGIN(responsible_dirs, routerstatus_t *, hsdir_rs) {
