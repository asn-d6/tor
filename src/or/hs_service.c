...
hs_service_ht_eq(const hs_service_t *first, const hs_service_t *second)
{
  ...
  /* Simple key compare. */
  if (!ed25519_pubkey_eq(&first->keys.identity_pk,
...
remove_service(hs_service_ht *map, hs_service_t *service)
{
  ...
  if (service == NULL ||
      ed25519_public_key_is_zero(&service->keys.identity_pk)) {
    return;
  }
                       ...
                       hs_service_descriptor_t **desc)
{
  ...
  if (s && ip) {
    *ip = service_intro_point_find_by_ident(s, ident);
  }
...
get_node_from_intro_point(const hs_service_intro_point_t *ip)
{
  ...
  SMARTLIST_FOREACH_BEGIN(ip->base.link_specifiers,
    ...
    node = node_get_by_id((const char *) ls->u.legacy_id);
    break;
 ...
 * return NULL if the node can't be found for the intro point. If direct_conn
 * is set, the extend info is validated on if we can connect directly. */
                                 ...
                                 unsigned int direct_conn)
{
  ...
  if (node == NULL) {
    /* XXX: This should not really happen considering that we can only pick an
     * IP if the node actually exists. Maybe BUG()? */
    goto end;
  }

  info = extend_info_from_node(node, direct_conn);
  /* XXX: Same here, when picking an IP, we make sure we can actually get the
   * extend info so it should never be NULL. BUG()? */
 end:
  return info;
}
...
{
  tor_assert(service);
  /* XXX: This needs the circuitmap API to support a getter for the service
   * side and the service needs a list of all RP cookie it's serving so we can
   * find the circuits in the map. */
  return;
}
...
close_service_circuits(hs_service_t *service)
{
  ...
  /* XXX: Kill HSDir circuits as well? */
}
...
move_ephemeral_services(hs_service_ht *src, hs_service_ht *dst)
{
  ...
  for (iter = HT_START(hs_service_ht, src); iter != NULL; iter = next) {
    ...
    if (register_service(dst, s) < 0) {
      /* XXX: Maybe improve message with which HS has that key. */
...
load_service_keys(hs_service_t *service)
{
  ...
  /* We'll try to generate the keys if they are not found and in this case we
   * want the strongest randomness for it. The keypair will be written in
   * different files. */
  key_flags = INIT_ED_KEY_CREATE | INIT_ED_KEY_EXTRA_STRONG |
              INIT_ED_KEY_SPLIT;
  kp = ed_key_init_from_file(fname, key_flags, LOG_WARN, NULL, 0, 0, 0, NULL);
  if (!kp) {
    log_warn(LD_REND, "Unable to load/generate keys from %s", fname);
    goto end;
  }
                        ...
                        hs_service_descriptor_t *desc, time_t now)
{
  ...
  DIGEST256MAP_FOREACH(desc->intro_points.current, key,
    ...
    desc_ip->auth_key_cert = tor_cert_create(&desc->signing_kp,
    ...
    /* Copy link specifier. */
    desc_ip->link_specifiers = smartlist_new();
    SMARTLIST_FOREACH_BEGIN(ip->base.link_specifiers,
                            hs_desc_link_specifier_t *, ls) {
    ...
    if (ip->base.is_only_legacy) {
      ...
      ed25519_public_key_from_curve25519_public_key(&ed25519_pubkey,
                                                    ...
                                                    0);
      cross_cert = tor_cert_create(&desc->signing_kp,
                                   CERT_TYPE_CROSS_HS_IP_KEYS,
                                   &ed25519_pubkey, now,
                                   HS_DESC_CERT_LIFETIME,
                                   CERT_FLAG_INCLUDE_SIGNING_KEY);
      if (cross_cert == NULL) {
        log_warn(LD_REND, "Unable to create encryption key curve25519 cross "
                          "certiciate for service %s",
                 safe_str_client(service->onion_address));
        continue;
      }
                             ...
                             hs_service_descriptor_t *desc, time_t now)
{
  ...
  plaintext->version = service->version;
  plaintext->lifetime_sec = HS_DESC_MAX_LIFETIME;
                        ...
                        uint64_t time_period_num)
{
  ...
  /* XXX: Support offline key feature. */
...
build_all_descriptors(time_t now)
{
  FOR_EACH_SERVICE_BEGIN(service) {
    ...
    /* A next descriptor to NULL indicate that we need to build a fresh one
     * for the _next_ time period since it means we just booted or we just
     * rotated our descriptors and thus we are in the overlap period starting
     * 12h before the next time period starts. */
...
pick_intro_point(unsigned int direct_conn, smartlist_t *exclude_nodes)
{
  ...
  if (!node) {
    log_warn(LD_REND, "Unable to find a suitable node to be an "
             "introduction point.");
    goto err;
  }
  ...
  /* Create our objects and populate them with the node info. */
  ip = service_intro_point_new();
  ...
  if (node->rs && !node->rs->supports_ed25519_hs_intro) {
                          ...
                          hs_service_descriptor_t *desc, time_t now)
{
  ...
  if ((unsigned int) digest256map_size(desc->intro_points.current) <
    ...
    if (num != 0) {
      log_info(LD_REND, "Service %s just picked %u intro points. "
                        "Launching ESTABLISH_INTRO circuit shortly.",
               safe_str_client(service->onion_address), num);
      /* We'll build those introduction point into the descriptor once we have
       * confirmation that the circuits are opened and ready. */
    }
  }

  /* If we are about to upload this descriptor, update the revision counter to
   * reflect that it has been updated and thus should be considered new by the
                           ...
                           time_t now)
{
  ...
  DIGEST256MAP_FOREACH_MODIFY(intro_points->current, key,
    ...
    /* If we have no circuit, we might keep this intro point and retry to
     * connect depending on certain conditions. */
    if (ocirc == NULL) {
      ...
      /* By keeping the object in the current map, we'll retry later. */
...
launch_intro_point_circuits(hs_service_t *service, time_t now)
{
  ...
  FOR_EACH_DESCRIPTOR_BEGIN(service, desc) {
    DIGEST256MAP_FOREACH(desc->intro_points.current, key,
      ...
      /* Skip the intro point that already has an established circuit. This is
       * set once the intro circuit has opened. */
      if (ip->circuit_established) {
        continue;
      }

      ei = get_extend_info_from_intro_point(ip,
                                            service->config.is_single_onion);
      if (BUG(ei == NULL)) {
        /* XXX: Should we remove the intro point from descriptor or trigger
         * descriptor fetch...? */
        continue;
      }
      ...
      if (hs_circ_launch_intro_point(service, ip, ei, now) < 0) {
        log_warn(LD_REND, "Unable to launch intro circuit to node %s "
                 ...
                 safe_str_client(service->onion_address));
        /* We don't remove it from the descriptor so it can be retried. */
      }
      extend_info_free(ei);
    } DIGEST256MAP_FOREACH_END;
  } FOR_EACH_DESCRIPTOR_END;
...
can_service_launch_intro_circuit(hs_service_t *service, time_t now)
{
  ...
  /* XXX: This needs to have a dynamic formula for the max circuit per retry
   * because it depends on the number of IP we've configured. See
   * rend_max_intro_circs_per_period() and #21594. */
  ...
  if (service->state.num_intro_circ_launched < MAX_INTRO_CIRCS_PER_PERIOD) {
    goto allow;
  }
...
service_intro_circ_has_opened(origin_circuit_t *circ)
{
  ...
  if (hs_circ_service_intro_has_opened(service, ip, circ)) {
                                 ...
                                 size_t payload_len)
{
  ...
  if (hs_circ_handle_intro_established(service, circ, ip, payload,
                                       payload_len) < 0) {
    goto err;
  }

  /* Bump the number of established circuit for this descriptor. This value is
   * what indicates the upload scheduled event if we are ready to build the
   * intro point into the descriptor and upload. */
  desc = service_desc_find_by_intro(service, ip);
  tor_assert(desc);
  desc->num_established_intro_circ++;
...
hs_service_load_all_keys(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(hs_service_staging_list, hs_service_t *, service) {
    ...
    /* XXX: Load/Generate client authorization keys. */
