...
cleanup_intro_points(hs_service_t *service, time_t now)
{
   ...
   * returned leading to a use-after-free. So, we close them after the loop if
   * any. */
  smartlist_t *ips_to_close = smartlist_new();
  ...
  FOR_EACH_DESCRIPTOR_BEGIN(service, desc) {
    ...
    DIGEST256MAP_FOREACH_MODIFY(desc->intro_points.map, key,
      ...
      if (has_expired || node == NULL ||
        ...
        /* Remove intro point from descriptor map and add it to the list of ips
         * to close */
        MAP_DEL_CURRENT(key);
        smartlist_add(ips_to_close, ip);
      }
    } DIGEST256MAP_FOREACH_END;
  } FOR_EACH_DESCRIPTOR_END;

  /* Close any intros that need to be closed */
  SMARTLIST_FOREACH_BEGIN(ips_to_close, hs_service_intro_point_t *, ip) {
