...
/* XXX: So we re-define the string above in this ruleset. We can't pass a
 * const to those TX macros but if the above were macros, that would work,
 * should we switch or duplicate is OK? */
static token_rule_t hs_desc_token_table[] = {
  T1_START("hs-descriptor", R_HS_DESCRIPTOR, EQ(1), NO_OBJ),
  T1("descriptor-lifetime", R3_DESC_LIFETIME, EQ(1), NO_OBJ),
  T1("descriptor-signing-key-cert", R3_DESC_SIGNING_CERT, NO_ARGS, NEED_OBJ),
...
static token_rule_t hs_desc_intro_point_token_table[] = {
  ...
  T1("auth-key", R3_INTRO_AUTH_KEY, EQ(1), NEED_OBJ),
  T1("enc-key", R3_INTRO_ENC_KEY, GE(1), OBJ_OK),
  END_OF_TABLE
};
                ...
                size_t plaintext_len, uint8_t **encrypted_out)
{
  ...
  tor_assert(padded_len <= HS_DESC_PADDED_PLAINTEXT_MAX_LEN);
  tor_assert(!(padded_len % HS_DESC_PLAINTEXT_PADDING_MULTIPLE));
...
validate_encrypted_data_length(size_t len)
{
  ...
  if (len < HS_DESC_ENCRYPTED_MIN_LEN) {
    log_warn(LD_REND, "Length of descriptor's encrypted data is too small."
                      "Got %lu but minimum value is %d", len,
...
static size_t
decrypt_data(const hs_descriptor_t *desc, char **decrypted_out)
{
  ...
  if (encrypted_len % HS_DESC_PLAINTEXT_PADDING_MULTIPLE) {
    log_warn(LD_REND, "Length of encrypted data is invalid (%lu)",
             encrypted_len);
    goto err;
  }
  /* At the very end is the MAC. */
...
decode_introduction_point(const char *text, const char *end)
{
  ...
  if (!tor_mem_is_zero((const char *) ip->enc_key.curve25519.pubkey.public_key,
                       CURVE25519_PUBKEY_LEN)) {
    /* ntor */
    tor_cert_t *cert;
    //ed25519_public_key_t enc_pubkey;

    if (strcmp(tok->object_type, "ED25519 CERT")) {
      log_warn(LD_REND, "Introduction point ntor encryption key "
               "cross-certification has an unexpected format");
      goto err;
    }

    /* XXX not checking expiration; does that matter?
     * XXX Not checking that the signing key is the right signing key */
    cert = tor_cert_parse((const uint8_t *) tok->object_body,
                          tok->object_size);
    if (!cert) {
      log_warn(LD_REND, "Introduction point enc key is invalid");
      goto err;
    } else if (cert->cert_type != CERT_TYPE_HS_IP_ENC) {
      log_warn(LD_REND, "Invalid cert type %02x for introduction point enc "
               "cert", cert->cert_type);
      goto err;
    } else if (!cert->signing_key_included) {
      log_warn(LD_REND, "Introduction point enc key lacks a signing key");
      goto err;
    // XXX: ??? } else if (!tor_memeq(cert->signing_key.pubkey, , ED25519_PUBKEY_LEN)) {
    } else if (tor_cert_checksig(cert, &cert->signing_key, 0)) {
      log_warn(LD_REND, "Introduction point enc key signature is invalid");
      goto err;
    }

    // XXX need sign bit :/
    //if (ed25519_public_key_from_curve25519_public_key(&enc_pubkey, &xxx)) {
    //} else if (ip->enc_key_legacy) {
      /* RSA */
    //} else {
    //  tor_fragile_assert();
      /* XXX */
                     ...
                     const char *encoded_desc, size_t encoded_len)
{
  ...
  tor_asprintf(&sig_token, "\n%s ", str_signature);
  sig_start = tor_memstr(encoded_desc, encoded_len, sig_token);
  tor_free(sig_token);
  ...
  if (sig_end != (encoded_desc + encoded_len - 1)) {
    /* XXX: Should we log that extra data? */
                         ...
                         const char *encoded_desc, size_t encoded_len)
{
  ...
  tor_assert(tokens);
  tor_assert(desc);
  /* Imagine we have a v4 that adds one extra field to the descriptor, we'll
   * still use this to decode the descriptor thus explaining the ">=". */
   ...
   * the blinded public key. XXX: Does this object type exists in the
   * tokenizing process? */
  ...
  if (!desc->signing_key_cert) {
    log_warn(LD_REND, "Service descriptor signing cert is invalid (%s)",
             escaped(tok->object_body));
    goto err;
  }
  if (desc->signing_key_cert->cert_type != CERT_TYPE_HS_DESC_SIGN) {
    log_warn(LD_REND, "Service descriptor signing cert unknown type %02x",
             desc->signing_key_cert->cert_type);
    goto err;
  }
  if (!desc->signing_key_cert->signing_key_included) {
    log_warn(LD_REND, "Service descriptor signing cert lacks a signing key");
    goto err;
    /* XXX not checking expiration; does that matter? */
  }
  if (tor_cert_checksig(desc->signing_key_cert,
                        &desc->signing_key_cert->signing_key, 0)) {
    log_warn(LD_REND, "Service descriptor signing cert signature is invalid");
    goto err;
  }
  ...
  /* XXX: Blinded key validation. Is it possible to validate thet we have the
   * right blinded key that is from the signing key? */

  /* XXX should we allow unrecognized lines in the outer descriptor?  think
   * about forwards compatibility here.
   *   [dgoulet] yes we should, this function could be used to decode part of
   *   a descriptor with a future version for instance. */
...
static int
  (*decode_plaintext_handlers[])(
      ...
      size_t encoded_len) =
{
  NULL, /* v0 */ NULL, /* v1 */ NULL, /* v2 */
  desc_decode_plaintext_v3,
};

/* XXX: Wrong comment. FIXUP
 * Fully decode the given descriptor plaintext and on success provide a
 * pointer to a filled-in hs_descriptor_t in desc_out.
 *
 * Returns 0 and a newly allocated hs_descriptor_t as desc_out on success. On
 * error, XXX we need an enum of error cases. */
int
                         ...
                         hs_desc_plaintext_data_t *plaintext)
{
  ...
  if (tokenize_string(area, encoded, encoded + encoded_len, tokens,
                      hs_desc_token_table, 0) < 0) {
    log_warn(LD_REND, "Service descriptor is not parseable");
    goto err;
  }

  /* XXX: Should this be version specific? Check if we have at least the
   * minimum amount of required fields. */
  if (smartlist_len(tokens) < HS_DESC_PLAINTEXT_MIN_FIELDS) {
    log_warn(LD_REND, "Service descriptor is missing required fields");
    goto err;
  }

  /* Get the version of the descriptor which is the first field of the
   * descriptor. From there, we'll decode the right descriptor version. */
  ...
  /* Extra precaution. No handlers for the supported version should never
   * happened else we forgot to add it but we bumped the version. */
                         ...
                         hs_desc_encrypted_data_t *desc_out)
{
 ...
 err:
  tor_assert(result < 0);
  desc_encrypted_data_free_contents(desc_out);
