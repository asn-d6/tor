...
static token_rule_t hs_desc_intro_point_v3_token_table[] = {
  ...
  T01(str_ip_legacy_key, R3_INTRO_LEGACY_KEY, NO_ARGS, NEED_KEY_1024),
...
decode_introduction_point(const hs_descriptor_t *desc, const char *start)
{
  ...
  if (!strcmp(tok->args[0], "ntor")) {
    ...
    if (tok->n_args != 2 || tok->object_body) {
      log_warn(LD_REND, "Introduction point ntor enc-key format is invalid");
      goto err;
    }
...
hs_descriptor_free_intro_points(hs_descriptor_t *desc)
{
  ...
  if (ips) {
    SMARTLIST_FOREACH_BEGIN(ips, hs_desc_intro_point_t *, ip) {
      desc_intro_point_free(ip);
    } SMARTLIST_FOREACH_END(ip);
    smartlist_clear(ips);
  }
