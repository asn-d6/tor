...
/* An entry that keeps enough relay information to correlate a failure between
 * connections to the same identity digest. */
...
note_or_connect_failed(const or_connection_t *or_conn)
{
   ...
   * a failure inside the node_t. If they do not match, we note it as an
   * unknown.
   *
   * This is to avoid someone sending extend requests for a known relay
   * identity digest but with the wrong IP/Port. */
...
static int
or_can_connect(const or_connection_t *or_conn)
{
  time_t now, cutoff;
  const time_t *ptr_since_ts = NULL;
  ...
  ocf = or_connect_failure_find(or_conn);
  if (ocf) {
    ptr_since_ts = &ocf->unable_connect_since_ts;
  } else {
    ...
    if (node) {
      ptr_since_ts = &node->unable_connect_since_ts;
    }
  }
   ...
   * let it connect. */
  if (!ptr_since_ts || *ptr_since_ts <= cutoff) {
    goto connect;
  }
