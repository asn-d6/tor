...
setup_intro_circ_auth_key(origin_circuit_t *circ)
{
  ...
  SMARTLIST_FOREACH_BEGIN(desc->encrypted_data.intro_points,
    ...
    SMARTLIST_FOREACH_BEGIN(ip->link_specifiers,
      ...
      /* We identifier only by ed25519. No more SHA1! :) */
      if (lspec->type != LS_ED25519_ID) {
        continue;
      }
      if (fast_memneq(circ->build_state->chosen_exit->ed_identity.pubkey,
                      lspec->u.ed25519_id, ED25519_PUBKEY_LEN)) {
        continue;
      }
...
client_intro_circ_has_opened(origin_circuit_t *circ)
{
  ...
  log_info(LD_REND, "Introduction circuit has opened. Attaching streams...");
 ...
 * usable. */
static extend_info_t *
client_get_random_intro(const hs_descriptor_t *desc)
{
                          ...
                          origin_circuit_t *rend_circ)
{
  ...
  log_info(LD_REND, "Sending INTRODUCE1 cell to service %s",
  ...
  if (desc == NULL || !hs_client_any_intro_points_usable(desc)) {
    ...
    {
      connection_t *conn;
      while ((conn = connection_get_hs_waiting(service_identity_pk))) {
...
extend_info_t *
hs_client_get_random_intro(hs_ident_edge_conn_t *ident)
{
  ...
  /* Pick a random intro point from the descriptor. */
  ei = client_get_random_intro(desc);
