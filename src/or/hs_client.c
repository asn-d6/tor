 ...
 *  descriptor by launching a dir connection to <b>hsdir</b>. */
static int
                               ...
                               const routerstatus_t *hsdir)
{
  ...
  if (retval < 0) {
    log_warn(LD_BUG, "Failed to base64 pubkey");
    return -1;
  }
  ...
  directory_request_set_hs_ident(req, &hs_conn_dir_ident);
  directory_initiate_request(req);
  directory_request_free(req);

  log_warn(LD_GENERAL, "Launched HS desc dirreq to %s", hsdir->nickname);
...
{
  uint64_t current_time_period = hs_get_time_period_num(approx_time());
  smartlist_t *responsible_hsdirs_nodes = smartlist_new();
  smartlist_t *responsible_hsdirs_rs = smartlist_new();
  ed25519_public_key_t blinded_pubkey;
  char base64_blinded_pubkey[ED25519_BASE64_LEN + 1];
  ...
  hs_get_responsible_hsdirs(&blinded_pubkey, current_time_period, 0, 1,
                            responsible_hsdirs_nodes);

  log_warn(LD_REND, "Found %d responsible HSDirs and about to fetch",
           smartlist_len(responsible_hsdirs_nodes));

  /* The responsible_hsdirs_nodes smartlist currently contains node_t*
     elements, but hs_pick_hsdir() expects it with routerstatus_t*
     elements. Make it happen. */
  SMARTLIST_FOREACH_BEGIN(responsible_hsdirs_nodes, node_t *, dir_node) {
    if (BUG(!dir_node->rs)) {
      continue;
    }
    smartlist_add(responsible_hsdirs_rs, dir_node->rs);
  } SMARTLIST_FOREACH_END(dir_node);

  /* Pick an HSDir from the responsible ones */
  hsdir_rs = hs_pick_hsdir(responsible_hsdirs_rs, base64_blinded_pubkey);

  smartlist_free(responsible_hsdirs_nodes);
 ...
 *  <b>desc</b>. */
int
hs_any_intro_points_usable(const hs_descriptor_t *desc)
{
...
/** Launch a connection to a hidden service directory to fetch a v3 rendezvous
 * service descriptor using <b>onion_identity_pk</b> to get the necessary keys.
 *
 * On success, 1 is returned. If no hidden service is left to ask, return 0.
...
hs_client_refetch_v3_renddesc(const ed25519_public_key_t *onion_identity_pk)
{
  ...
  if (!get_options()->FetchHidServDescriptors) {
    log_warn(LD_REND, "We received an onion address for a v3 rendezvous "
  ...
  /* Check if fetching a desc for this HS is useful to us right now */
  {
    ...
    cached_desc = hs_cache_lookup_as_client(onion_identity_pk);
    if (cached_desc && hs_any_intro_points_usable(cached_desc)) {
      log_warn(LD_GENERAL, "We would fetch a v3 desc but we already "
               "have a useable desc.");
      return 0;
    }
  }

  return fetch_v3_desc(onion_identity_pk);
}
